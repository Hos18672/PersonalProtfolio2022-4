    ...
    if (item.webLink) {
      li.innerHTML = `<time class="time">${item.date}</time>
            <div  class="project-item">
              <img class="portfolio-img" src=${encodeURI(item.img)} alt="${item.title}" aria-hidden="true" aria-label="${item.title}"  title="${item.title}" />
...
function setSkillsData(data) {
  ...
  data.forEach((item) => {
    let div = document.createElement("div");
    div.innerHTML = `<img src=${encodeURI(item.img)} aria-hidden="true" alt=${item.title} />
